...
6. **MLX constraint grading.** When the mission brief marks the MLX constraint workflow mandatory, require the harness-owned constraint gate to pass. Check that the useful end-to-end metric, matched baseline, critical-path constraint, Roofline evidence, exploit/subordinate/elevate decisions, parity, and next constraint support the claimed stage. Reject isolated kernel, memory, or utilization wins.
7. **Plan health.** Do the milestones still match what the evidence says the goal needs? If findings invalidate the plan, say so and pick `replan`.
8. **Reasoning-state hygiene.** Did the worker preserve a clean problem state: known facts, unknowns, candidate hypotheses, chosen test, observation, belief update, and a pre-registered next discriminating test with expected outcomes? Apply the load-bearing test to the belief update: if it would read the same under the opposite observation, the observation is doing no work — flag it. Flag missing or unsupported state transitions in steering notes.
9. **Memory hygiene.** Did the worker's notebook update drop durable facts, ruled-out hypotheses, or human guidance? Flag it in steering notes.
